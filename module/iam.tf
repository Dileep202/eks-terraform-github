...
resource "random_integer" "random_suffixname" {
    min = 1000
    max = 9999
}

resource "aws_iam_role" "eks_cluster_role_name"{
     count = var.is_eks_role_enabled ? 1 :  0 
    name = "${var.cluster_name}-role-${random_integer.random_suffixname.result}"
...
resource "aws_iam_role_policy_attachment" "AmazonEKSClusterPolicy" {
    ...
    role = aws_iam_role.eks_cluster_role_name[count.index].name
}
